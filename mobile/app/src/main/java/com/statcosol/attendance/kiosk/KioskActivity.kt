...
import com.statcosol.attendance.api.SubmitKioskTicketRequest
...
class KioskActivity : AppCompatActivity() {
    ...
    private var capture: KioskCameraSession? = null
    ...
    private suspend fun handleFace(probe: FloatArray, liveness: Double) {
        ...
        val subjectKey = match.entry.subjectId
        ...
        lifecycleScope.launch {
            val resp = try {
                withContext(Dispatchers.IO) {
                    app.mobileApi.requestLivenessChallenge(match.entry.subjectId)
                }
            } catch (e: Exception) {
    ...
    private fun maybeReportRepeatedNoMatch(liveness: Double) {
        ...
        android.util.Log.i("KioskActivity", "repeated no-match count=$consecutiveNoMatchCount liveness=$liveness")
    }
    ...
    private suspend fun recordPunch(match: RosterMatcher.Match, direction: String, liveness: Double) {
        ...
        val probe = pendingChallengeProbe ?: return
        ...
        val embeddingB64 = FaceEmbedder.encodeEmbeddingB64(probe)
        val q = QueuedPunch(
        ...
        val punchAccepted = try {
            withContext(Dispatchers.IO) {
                val resp = app.mobileApi.postPunch(
                    RecordPunchRequest(
                        embeddingB64 = embeddingB64,
                        direction = direction,
                        livenessNonce = nonce,
    ...
    private fun handleRejectedPunch(match: RosterMatcher.Match, message: String) {
        ...
        android.util.Log.w(
            "KioskActivity",
            "punch rejected for ${match.entry.displayName}: ${message.take(300)}",
        )
    }
    ...
    private fun pollEnrollTicket() {
        ...
        lifecycleScope.launch {
            val t = try {
                withContext(Dispatchers.IO) { app.mobileApi.fetchPendingKioskTicket() }
    ...
    private fun beginKioskEnrollment(t: KioskEnrollTicket) {
        ...
        lifecycleScope.launch {
            ...
            runOnUiThread {
                binding.statusText.text = getString(
                    R.string.kiosk_liveness_prompt_with_name,
                    t.subjectName ?: getString(R.string.kiosk_unknown_employee),
                    getString(promptResFor(challenge)),
                )
    ...
    private fun submitKioskEnrollment() {
        ...
        lifecycleScope.launch {
            try {
                val nonce = enrollChallengeNonce ?: return@launch
                val body = SubmitKioskTicketRequest(
                    ticketId = t.id,
                    embeddingFrames = enrollFrames.map { FaceEmbedder.encodeEmbeddingB64(it) },
                    livenessNonce = nonce,
                    livenessChallengeType = enrollChallenge?.challenge?.wireName,
                    consentGiven = true,
                )
