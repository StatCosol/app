-- =============================================================
-- Sprint 1: Applicability Engine V2 — Seed Data
-- Real Indian labour compliance masters, rules, thresholds,
-- packages, and safety document codes
-- =============================================================
...
INSERT INTO unit_compliance_master (code, name, category, state_code, frequency, applies_to) VALUES
  ...
  -- Shops & Establishments (State-level)
...
-- ─── Applicability Rules ─────────────────────────────────────
-- Rules reference thresholds by code (data-driven, not hardcoded)

-- EPF: employee_total >= PF_COVERAGE_THRESHOLD
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'EPF coverage by headcount', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","threshold":"PF_COVERAGE_THRESHOLD"}]}'::jsonb
FROM unit_compliance_master WHERE code = 'EPF'
ON CONFLICT DO NOTHING;

-- ESIC: employee_total >= ESIC_COVERAGE_THRESHOLD
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'ESIC coverage by headcount', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","threshold":"ESIC_COVERAGE_THRESHOLD"}]}'::jsonb
FROM unit_compliance_master WHERE code = 'ESIC'
ON CONFLICT DO NOTHING;

-- Gratuity: employee_total >= 10
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Gratuity by headcount', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":10}]}'::jsonb
FROM unit_compliance_master WHERE code = 'GRATUITY'
ON CONFLICT DO NOTHING;

-- Bonus: employee_total >= 20
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Bonus by headcount', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":20}]}'::jsonb
FROM unit_compliance_master WHERE code = 'BONUS'
ON CONFLICT DO NOTHING;

-- Min Wages: always applicable (employee_total >= 1)
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Minimum Wages universal', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":1}]}'::jsonb
FROM unit_compliance_master WHERE code = 'MIN_WAGES'
ON CONFLICT DO NOTHING;

-- POW: always applicable
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Payment of Wages universal', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":1}]}'::jsonb
FROM unit_compliance_master WHERE code = 'POW'
ON CONFLICT DO NOTHING;

-- Maternity: employee_total >= 10
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Maternity Benefit by headcount', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":10}]}'::jsonb
FROM unit_compliance_master WHERE code = 'MATERNITY'
ON CONFLICT DO NOTHING;

-- POSH: always applicable
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'POSH universal', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":1}]}'::jsonb
FROM unit_compliance_master WHERE code = 'POSH'
ON CONFLICT DO NOTHING;

-- Employee Compensation: always applicable
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Employee Compensation universal', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":1}]}'::jsonb
FROM unit_compliance_master WHERE code = 'EE_COMP'
ON CONFLICT DO NOTHING;

-- CLRA: contractors_count >= 1 AND contract_workers_total >= CLRA_THRESHOLD
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'CLRA by contractor count', 10,
  id, 'ENABLE',
...
-- ISMW: contract_workers_total >= ISMW_THRESHOLD
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'ISMW by migrant worker count', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"contract_workers_total","op":">=","threshold":"ISMW_THRESHOLD"}]}'::jsonb
FROM unit_compliance_master WHERE code = 'ISMW'
ON CONFLICT DO NOTHING;

-- Equal Remuneration: always applicable
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Equal Remuneration universal', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":1}]}'::jsonb
FROM unit_compliance_master WHERE code = 'EQUAL_REMUN'
ON CONFLICT DO NOTHING;

-- Factories Act: establishment_type == FACTORY AND employee_total >= FACTORIES_ACT_THRESHOLD
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Factories Act for factories', 10,
  id, 'ENABLE',
...
-- BOCW: is_bocw_project == true
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'BOCW for construction projects', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"is_bocw_project","op":"==","value":true}]}'::jsonb
FROM unit_compliance_master WHERE code = 'BOCW'
ON CONFLICT DO NOTHING;

-- Canteen: employee_total >= CANTEEN_THRESHOLD AND establishment_type == FACTORY
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Canteen for large factories', 10,
  id, 'ENABLE',
...
-- Creche: employee_female >= CRECHE_FEMALE_THRESHOLD OR employee_total >= CRECHE_THRESHOLD
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Creche by female count or total', 10,
  id, 'ENABLE',
...
-- Shops & Establishments: establishment_type == ESTABLISHMENT
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'S&E for establishments', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"establishment_type","op":"==","value":"ESTABLISHMENT"}]}'::jsonb
FROM unit_compliance_master WHERE code = 'SHOPS_EST'
ON CONFLICT DO NOTHING;

-- Return rules (linked to parent compliance)
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'EPF Return when EPF applicable', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","threshold":"PF_COVERAGE_THRESHOLD"}]}'::jsonb
...
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'ESIC Return when ESIC applicable', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","threshold":"ESIC_COVERAGE_THRESHOLD"}]}'::jsonb
...
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Bonus Return when Bonus applicable', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"employee_total","op":">=","value":20}]}'::jsonb
...
-- License rules
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'Factory License for factories', 10,
  id, 'ENABLE',
...
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'S&E Registration for establishments', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"establishment_type","op":"==","value":"ESTABLISHMENT"}]}'::jsonb
...
INSERT INTO applicability_rule (name, priority, target_compliance_id, effect, conditions_json)
SELECT 'CLRA License when CLRA applicable', 10,
  id, 'ENABLE',
  '{"all":[{"fact":"contractors_count","op":">=","value":1},{"fact":"contract_workers_total","op":">=","threshold":"CLRA_THRESHOLD"}]}'::jsonb
...
INSERT INTO compliance_package (code, name, applies_to) VALUES
  ('DEFAULT_INDIA', 'Default India Compliance Package', 'BOTH')
ON CONFLICT (code) DO NOTHING;

-- Link ALL compliance masters to the default package
...
-- Link ALL rules to the default package
...
-- ─── Safety Document Master Updates ──────────────────────────
-- Add codes and engine metadata to existing safety_document_master rows
-- (The migration already added the columns; now populate them)

UPDATE safety_document_master SET code = 'FIRE_NOC'         WHERE document_name ILIKE '%fire%noc%'          AND code IS NULL;
UPDATE safety_document_master SET code = 'FACTORY_LICENSE'   WHERE document_name ILIKE '%factory%license%'   AND code IS NULL;
UPDATE safety_document_master SET code = 'POLLUTION_CERT'    WHERE document_name ILIKE '%pollution%'         AND code IS NULL;
UPDATE safety_document_master SET code = 'ELECTRICAL_AUDIT'  WHERE document_name ILIKE '%electrical%audit%'  AND code IS NULL;
UPDATE safety_document_master SET code = 'SAFETY_POLICY'     WHERE document_name ILIKE '%safety%policy%'     AND code IS NULL;
UPDATE safety_document_master SET code = 'FIRST_AID'         WHERE document_name ILIKE '%first%aid%'         AND code IS NULL;
UPDATE safety_document_master SET code = 'FIRE_DRILL'        WHERE document_name ILIKE '%fire%drill%'        AND code IS NULL;

-- Mark hazardous-only documents
UPDATE safety_document_master SET is_hazardous_only = true WHERE code IN ('ELECTRICAL_AUDIT');

-- Done
SELECT 'Sprint 1 V2 seed data loaded successfully' AS status;
