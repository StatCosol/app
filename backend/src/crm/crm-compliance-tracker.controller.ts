...
export class CrmComplianceTrackerController {
  ...
  async mcd(@Req() req: any, @Query() query: any) {
    ...
    const rows = await this.db.many(
      `WITH crm_clients AS (
       ...
       crm_branches AS (
         SELECT b.id AS branch_id, b.branch_name, b.client_id
         FROM branches b
         JOIN crm_clients cc ON cc.client_id = b.client_id
         WHERE b.is_active = TRUE
           AND ($4::uuid IS NULL OR b.client_id = $4)
       )
       SELECT
    ...
    const scope = await this.db.many(
      `SELECT b.id
       ...
       JOIN client_assignments_current cac
         ON cac.client_id = b.client_id
         AND cac.assigned_to_user_id = $1
         AND cac.assignment_type = 'CRM'
       WHERE b.id = $2::uuid`,
      [crmUserId, branchId],
    );
    if (!scope.length) return { ok: false, message: 'Branch not in your scope' };
  ...
  async auditClosures(@Req() req: any, @Query() query: any) {
    ...
    const rows = await this.db.many(
      `WITH crm_clients AS (
       ...
       GROUP BY a.id, a.audit_code, a.audit_type, c.client_name, b.branch_name, u.name
  ...
  async closeObservation(
    ...
    if (!scope.length) return { ok: false, message: 'Observation not in your scope' };
