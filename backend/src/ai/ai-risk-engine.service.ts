...
export class AiRiskEngineService {
  ...
  async gatherComplianceData(clientId: string): Promise<ComplianceData> {
    ...
    const client = await this.dataSource.query(
      `SELECT c.id, c.client_name, COUNT(DISTINCT b.id) as branch_count
       FROM clients c LEFT JOIN branches b ON b.client_id = c.id AND b.is_active = TRUE
    ...
    const stateResult = await this.dataSource.query(
      `SELECT state, COUNT(*) as cnt FROM branches WHERE client_id = $1 AND is_active = TRUE
       GROUP BY state ORDER BY cnt DESC LIMIT 1`,
      [clientId],
    ).catch(() => [{ state: 'Unknown' }]);
    const state = stateResult[0]?.state || 'Unknown';

    // MCD compliance
    const mcdResult = await this.dataSource.query(
    ...
    // PF / ESI pending
    const pfResult = await this.dataSource.query(
      `SELECT
    ...
    const pfDelayResult = await this.dataSource.query(
    ...
    const obsResult = await this.dataSource.query(
    ...
    const contractorResult = await this.dataSource.query(
      `SELECT COUNT(DISTINCT cu.id) as total_contractors
       FROM users cu
       JOIN branches b ON b.client_id = $1
    ...
    const expiredDocResult = await this.dataSource.query(
      `SELECT COUNT(DISTINCT cd.contractor_user_id)::int as expired_count
       FROM contractor_documents cd
       INNER JOIN branches b ON b.id = cd.branch_id AND b.client_id = $1
    ...
    const docCompletionResult = await this.dataSource.query(
      `SELECT
       ...
       INNER JOIN branches b ON b.id = crd.branch_id AND b.client_id = $1
    ...
    // Pending returns
    const returnResult = await this.dataSource.query(
    ...
    return {
      ...
      totalMcdItems,
      uploadedMcdItems,
      mcdCompliancePercent: totalMcdItems > 0 ? Math.round((uploadedMcdItems / totalMcdItems) * 100) : 100,
      ...
      contractorsWithExpiredDocs: Number(expiredDocResult[0]?.expired_count || 0),
      contractorDocCompletionPercent: totalRequired > 0 ? Math.round((uploadedDocs / totalRequired) * 100) : 100,
      overdueTasks,
      totalTasks,
      overduePercent: totalTasks > 0 ? Math.round((overdueTasks / totalTasks) * 100) : 0,
      pendingReturns: Number(returnResult[0]?.pending || 0),
    };
  }

  /** Calculate a deterministic base risk score from data (no AI needed) */
  calculateBaseRiskScore(data: ComplianceData): { score: number; factors: any[] } {
    const factors: any[] = [];
    let score = 0;
    ...
    if (mcdPoints > 0) factors.push({ factor: 'MCD Compliance Gap', weight: 25, value: `${data.mcdCompliancePercent}%`, detail: `${data.uploadedMcdItems}/${data.totalMcdItems} items uploaded` });
    score += mcdPoints;

    // PF delay (max 20 points)
    const pfPoints = Math.min(20, data.pfDelayMonths * 5);
    if (pfPoints > 0) factors.push({ factor: 'PF Payment Delay', weight: 20, value: `${data.pfDelayMonths} months`, detail: `${data.pfPendingEmployees} employees pending PF registration` });
    score += pfPoints;

    // ESI pending (max 10 points)
    const esiPoints = Math.min(10, Math.round((data.esiPendingEmployees / Math.max(data.employeeCount, 1)) * 10));
    if (esiPoints > 0) factors.push({ factor: 'ESI Registration Gap', weight: 10, value: `${data.esiPendingEmployees} pending`, detail: `Out of ${data.employeeCount} employees` });
    score += esiPoints;

    // Audit non-compliances (max 20 points)
    const ncPoints = Math.min(20, data.nonComplianceCount * 4);
    if (ncPoints > 0) factors.push({ factor: 'Audit Non-Compliances', weight: 20, value: `${data.nonComplianceCount} high/critical`, detail: `${data.totalAudits} total audits` });
    score += ncPoints;

    // Overdue tasks (max 15 points)
    const taskPoints = Math.min(15, Math.round(data.overduePercent * 0.15));
    if (taskPoints > 0) factors.push({ factor: 'Overdue Compliance Tasks', weight: 15, value: `${data.overduePercent}%`, detail: `${data.overdueTasks}/${data.totalTasks} tasks overdue` });
    score += taskPoints;

    // Pending returns (max 10 points)
    const returnPoints = Math.min(10, data.pendingReturns * 2);
    if (returnPoints > 0) factors.push({ factor: 'Pending Returns', weight: 10, value: `${data.pendingReturns} pending`, detail: 'Unfiled statutory returns' });
    score += returnPoints;
  ...
  async runAssessment(clientId: string, assessedBy: string, assessmentType = 'COMPLIANCE'): Promise<AiRiskAssessmentEntity> {
    ...
    if (isReady) {
      ...
      if (result) {
        try {
          ...
          this.logger.warn('Failed to parse AI risk response, using base score');
        }
      }
    }
    ...
    const inspectionProbability = Math.min(95, finalScore * 0.85 + (data.pfDelayMonths > 2 ? 15 : 0));
  ...
  private async generateInsights(assessment: AiRiskAssessmentEntity, data: ComplianceData) {
    ...
    if (data.pfDelayMonths >= 3) {
      insights.push({
        ...
        description: `${data.clientName} has ${data.pfDelayMonths} months of PF delay with ${data.pfPendingEmployees} employees pending registration. ` +
        ...
        data: { pfDelayMonths: data.pfDelayMonths, pfPendingEmployees: data.pfPendingEmployees },
    ...
    if (data.mcdCompliancePercent < 80) {
      insights.push({
        ...
        description: `Only ${data.uploadedMcdItems} of ${data.totalMcdItems} MCD items uploaded. ` +
        ...
        data: { mcdPercent: data.mcdCompliancePercent, pending: data.totalMcdItems - data.uploadedMcdItems },
    ...
    if (data.nonComplianceCount >= 2) {
      insights.push({
        ...
        description: `Multiple non-compliance points detected across ${data.totalAudits} audits. ` +
          `Penalty risk increases with each unresolved observation.`,
        data: { ncCount: data.nonComplianceCount, totalAudits: data.totalAudits },
    ...
    if (data.overduePercent > 30) {
      insights.push({
        ...
        description: `${data.overdueTasks} of ${data.totalTasks} compliance tasks are overdue. ` +
          `This trend suggests compliance will fall below 80% next month if not addressed.`,
        data: { overduePercent: data.overduePercent, overdueTasks: data.overdueTasks },
        validUntil: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
      });
    }

    if (insights.length > 0) {
      await this.insightRepo.save(insights.map(i => this.insightRepo.create(i)));
    }
  }
  ...
  private generateFallbackSummary(data: ComplianceData, score: number): string {
    ...
    const parts: string[] = [`${data.clientName} — ${level} risk (score: ${score}/100).`];

    if (data.pfDelayMonths > 0) parts.push(`PF payments delayed by ${data.pfDelayMonths} months.`);
    if (data.mcdCompliancePercent < 100) parts.push(`MCD compliance at ${data.mcdCompliancePercent}%.`);
    if (data.nonComplianceCount > 0) parts.push(`${data.nonComplianceCount} high/critical audit NCs.`);
    if (data.overdueTasks > 0) parts.push(`${data.overdueTasks} overdue compliance tasks.`);
    if (data.pendingReturns > 0) parts.push(`${data.pendingReturns} unfiled statutory returns.`);

    return parts.join(' ');
  }

  private generateFallbackRecommendations(data: ComplianceData, factors: any[]): any[] {
    const recs: any[] = [];
    if (data.pfDelayMonths > 0) {
      recs.push({ priority: 1, action: 'Clear overdue PF payments immediately with interest calculation', impact: 'Reduces EPFO inspection risk by 30–40%' });
    }
    if (data.pfPendingEmployees > 0) {
      recs.push({ priority: 2, action: `Register ${data.pfPendingEmployees} employees for PF`, impact: 'Eliminates penalty for non-registration under EPF Act' });
    }
    if (data.mcdCompliancePercent < 100) {
      recs.push({ priority: 3, action: `Upload ${data.totalMcdItems - data.uploadedMcdItems} pending MCD items`, impact: `Raise compliance to 100% from current ${data.mcdCompliancePercent}%` });
    }
    if (data.nonComplianceCount > 0) {
      recs.push({ priority: 4, action: 'Resolve high/critical audit observations', impact: 'Reduces cumulative penalty exposure' });
    }
    if (data.overdueTasks > 0) {
      recs.push({ priority: 5, action: `Complete ${data.overdueTasks} overdue compliance tasks`, impact: 'Prevents further escalation and non-compliance accumulation' });
    }
    return recs;
  }

  private generateFallbackPredictions(data: ComplianceData, score: number): Record<string, any> {
    return {
      inspectionTimeframe: score >= 60 ? '3–6 months' : score >= 30 ? '6–12 months' : '12+ months',
      trendDirection: data.overduePercent > 20 ? 'WORSENING' : data.overduePercent > 10 ? 'STABLE' : 'IMPROVING',
  ...
  async getLatestAssessment(clientId: string): Promise<AiRiskAssessmentEntity | null> {
  ...
  async getAssessmentHistory(clientId: string, limit = 10): Promise<AiRiskAssessmentEntity[]> {
  ...
  async dismissInsight(insightId: string, userId: string): Promise<void> {
    await this.insightRepo.update(insightId, { isDismissed: true, dismissedBy: userId });
  }
  ...
  async getPlatformRiskSummary(): Promise<any> {
    ...
    return {
      ...(summary[0] || {}),
      activeInsights: insightCounts.reduce((acc: any, r: any) => ({ ...acc, [r.severity]: Number(r.cnt) }), {}),
    };
  }
  ...
  async buildBranchRiskData(branchId: string, year: number, month: number): Promise<BranchRiskData> {
    // Branch info — try client_branches first (primary table), then branches as fallback
    let branchInfo = await this.dataSource.query(
      `SELECT id, branchname AS branch_name, clientid AS client_id FROM client_branches WHERE id = $1`,
      [branchId],
    ).catch(() => []);
    if (!branchInfo.length) {
      branchInfo = await this.dataSource.query(
        `SELECT b.id, b.branch_name, b.client_id FROM branches b WHERE b.id = $1`,
        [branchId],
      ).catch(() => []);
    }
    ...
    const esiResult = await this.dataSource.query(
      `SELECT COUNT(*) as applicable,
    ...
    const avgDaysPfPending = Math.round(Number(pfDelayResult[0]?.avg_days || 0));
    ...
    const returnsPendingResult = await this.dataSource.query(
    ...
    const hasOverdue = overdueResult[0]?.has_overdue === true || overdueResult[0]?.has_overdue === 't';
    ...
    return {
      ...
      pfApplicable,
      pfRegistered,
      pfPercent: pfApplicable > 0 ? Math.round((pfRegistered / pfApplicable) * 100) : 100,
      esiApplicable,
      esiRegistered,
      esiPercent: esiApplicable > 0 ? Math.round((esiRegistered / esiApplicable) * 100) : 100,
      requiredDocs,
      uploadedDocs,
      docPercent: requiredDocs > 0 ? Math.round((uploadedDocs / requiredDocs) * 100) : 100,
      avgDaysPfPending,
      auditNcCount,
      returnsPending,
      hasOverdue,
  ...
  computeBranchRiskScore(data: BranchRiskData): { score: number; level: string; factors: any[] } {
    const factors: any[] = [];
    let score = 0;
    ...
    const mcdPoints = Math.min(25, Math.round(mcdGap * 0.25));
    if (mcdPoints > 0) factors.push({ factor: 'MCD Compliance Gap', weight: 25, scored: mcdPoints, detail: `${data.mcdDone}/${data.mcdTotal} items done (${data.mcdPercent}%)` });
    score += mcdPoints;

    // PF registration gap (max 20 points)
    const pfGap = 100 - data.pfPercent;
    const pfPoints = Math.min(20, Math.round(pfGap * 0.20));
    if (pfPoints > 0) factors.push({ factor: 'PF Registration Gap', weight: 20, scored: pfPoints, detail: `${data.pfRegistered}/${data.pfApplicable} registered (${data.pfPercent}%)` });
    score += pfPoints;

    // ESI registration gap (max 10 points)
    const esiGap = 100 - data.esiPercent;
    const esiPoints = Math.min(10, Math.round(esiGap * 0.10));
    if (esiPoints > 0) factors.push({ factor: 'ESI Registration Gap', weight: 10, scored: esiPoints, detail: `${data.esiRegistered}/${data.esiApplicable} registered (${data.esiPercent}%)` });
    score += esiPoints;

    // Contractor document gap (max 15 points)
    const docGap = 100 - data.docPercent;
    const docPoints = Math.min(15, Math.round(docGap * 0.15));
    if (docPoints > 0) factors.push({ factor: 'Contractor Doc Gap', weight: 15, scored: docPoints, detail: `${data.uploadedDocs}/${data.requiredDocs} uploaded (${data.docPercent}%)` });
    score += docPoints;
    ...
    if (delayPoints > 0) factors.push({ factor: 'PF Delay Severity', weight: 15, scored: delayPoints, detail: `${data.avgDaysPfPending} avg days pending` });
    score += delayPoints;

    // Audit NCs (max 15 points — 3 per NC)
    const ncPoints = Math.min(15, data.auditNcCount * 3);
    if (ncPoints > 0) factors.push({ factor: 'Audit NCs (High/Critical)', weight: 15, scored: ncPoints, detail: `${data.auditNcCount} non-compliances` });
    score += ncPoints;
  ...
  async getBranchRiskSnapshot(branchId: string, year: number, month: number) {
    ...
    const ncBreakdown = await this.dataSource.query(
      `SELECT
    ...
    const inspectionProbability = score >= 75 ? 80 : score >= 50 ? 60 : score >= 25 ? 35 : 15;
  ...
  async runBranchAssessment(params: {
    ...
    const request = await this.requestLog.createRequest({
      ...
      payload: { branchId: params.branchId, year: params.year, month: params.month },
    ...
    try {
      ...
      const data = await this.buildBranchRiskData(params.branchId, params.year, params.month);
      ...
      try {
        const aiResult = await this.aiCore.complete(
          RISK_SYSTEM_PROMPT,
          JSON.stringify({ branchData: data, riskScore: score, riskLevel: level, factors }),
        );
        if (aiResult) {
          const parsed = JSON.parse(typeof aiResult === 'string' ? aiResult : aiResult.content || '{}');
  ...
  private generateBranchFindings(data: BranchRiskData): string[] {
    ...
    if (data.mcdPercent < 100) {
      findings.push(`MCD compliance: ${data.mcdDone}/${data.mcdTotal} items completed (${data.mcdPercent}%). ${data.mcdTotal - data.mcdDone} items pending.`);
    }
    if (data.pfPercent < 100) {
      findings.push(`PF registration: ${data.pfRegistered}/${data.pfApplicable} applicable employees registered (${data.pfPercent}%). ${data.pfApplicable - data.pfRegistered} employees unregistered.`);
    }
    if (data.esiPercent < 100) {
      findings.push(`ESI registration: ${data.esiRegistered}/${data.esiApplicable} applicable employees registered (${data.esiPercent}%).`);
    }
    if (data.docPercent < 100) {
      findings.push(`Contractor documents: ${data.uploadedDocs}/${data.requiredDocs} required documents uploaded (${data.docPercent}%).`);
    }
    if (data.avgDaysPfPending > 0) {
      findings.push(`PF registration delay: Average ${data.avgDaysPfPending} days pending for unregistered employees.`);
    }
    if (data.auditNcCount > 0) {
      findings.push(`Audit observations: ${data.auditNcCount} high/critical non-compliances in ${data.year}.`);
    }
    if (findings.length === 0) {
      findings.push('No significant compliance gaps detected for this branch and period.');
    }
  ...
  private generateBranchActions(data: BranchRiskData, factors: any[]): any[] {
    ...
    if (data.pfPercent < 100) {
      actions.push({ priority: priority++, action: `Register ${data.pfApplicable - data.pfRegistered} employees for PF`, impact: 'Eliminates EPF Act non-compliance penalty', urgency: data.avgDaysPfPending > 60 ? 'IMMEDIATE' : 'HIGH' });
    }
    if (data.mcdPercent < 80) {
      actions.push({ priority: priority++, action: `Complete ${data.mcdTotal - data.mcdDone} pending MCD items`, impact: `Raise MCD compliance from ${data.mcdPercent}% to 100%`, urgency: 'HIGH' });
    }
    if (data.esiPercent < 100) {
      actions.push({ priority: priority++, action: `Register ${data.esiApplicable - data.esiRegistered} employees for ESI`, impact: 'Eliminates ESI Act non-compliance risk', urgency: 'HIGH' });
    }
    if (data.docPercent < 100) {
      actions.push({ priority: priority++, action: `Upload ${data.requiredDocs - data.uploadedDocs} missing contractor documents`, impact: 'Achieves full contractor documentation compliance', urgency: 'MEDIUM' });
    }
    if (data.auditNcCount > 0) {
      actions.push({ priority: priority++, action: `Resolve ${data.auditNcCount} high/critical audit observations`, impact: 'Reduces accumulated penalty exposure', urgency: data.auditNcCount >= 3 ? 'IMMEDIATE' : 'HIGH' });
    }
  ...
  private generateBranchFallbackSummary(data: BranchRiskData, score: number, level: string): string {
    const parts: string[] = [`Branch "${data.branchName}" — ${level} risk (score: ${score}/100) for ${data.year}-${String(data.month).padStart(2, '0')}.`];
    if (data.mcdPercent < 100) parts.push(`MCD at ${data.mcdPercent}%.`);
    if (data.pfPercent < 100) parts.push(`PF registration at ${data.pfPercent}%.`);
    if (data.avgDaysPfPending > 30) parts.push(`PF delayed avg ${data.avgDaysPfPending} days.`);
