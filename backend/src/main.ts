import { uploadsAuthorization } from './files/uploads-authorization';
...
async function bootstrap() {
  ...
  app.use(
    '/uploads',
    uploadsAuthorization(jwtService, app.get(JwtStrategy), filesService),
  );
  ...
  // Exits with a clear error message so missing migrations are caught early.
  {
    ...
    // Contractor Sunday work C-off ledger (migration 20260926_contractor_comp_off).
    // Branch approval of contractor attendance writes to these tables.
    try {
      await ds.query(`
      ...
      `);
      logger.log('Schema patch: contractor C-off ledger OK');
    } catch (e: any) {
      logger.warn(
        `Schema patch contractor C-off ledger skipped: ${e?.message}`,
      );
    }
    ...
    try {
      const billingStmts: string[] = [
        ...
        `CREATE TABLE IF NOT EXISTS invoice_email_logs (id UUID PRIMARY KEY DEFAULT gen_random_uuid(),invoice_id UUID NOT NULL REFERENCES invoices(id) ON DELETE CASCADE,to_email TEXT NOT NULL,cc_email TEXT,bcc_email TEXT,subject VARCHAR(250) NOT NULL,body TEXT,sent_status VARCHAR(30) NOT NULL DEFAULT 'NOT_SENT',sent_at TIMESTAMP,sent_by UUID,failure_reason TEXT,created_at TIMESTAMP NOT NULL DEFAULT NOW())`,
        ...
        // BCC for invoice email. In this list rather than a block of its own:
        // a standalone patch ran before these tables were created, so on a
        // fresh database the first ALTER failed, its shared catch skipped the
        // second, and the CREATEs above then made both tables without the
        // column. Here the order is guaranteed and each statement is caught on
        // its own. Mirrors migrations/20260911_invoice_bcc_email.sql.
        `ALTER TABLE billing_clients ADD COLUMN IF NOT EXISTS bcc_email TEXT`,
