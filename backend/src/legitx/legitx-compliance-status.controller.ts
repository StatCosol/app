import { Controller, ForbiddenException, Get, Query, Req, UseGuards, ValidationPipe } from '@nestjs/common';
...
export class LegitxComplianceStatusController {
  ...
  async summary(
    @Req() req: any,
    @Query(new ValidationPipe({ transform: true, whitelist: true })) q: ComplianceStatusQueryDto,
  ) {
  ...
  private async buildParams(req: any, q: ComplianceStatusQueryDto) {
    ...
    const normalizedOffset = this.clamp(q.offset, 0, Number.MAX_SAFE_INTEGER, 0);
    ...
    if (user.clientId) {
      ...
      if (branchId && allowedBranchIds !== 'ALL' && !allowedBranchIds.includes(branchId)) {
        throw new ForbiddenException('You do not have access to this branch');
      }
      if (!branchId && allowedBranchIds !== 'ALL' && allowedBranchIds.length === 1) {
