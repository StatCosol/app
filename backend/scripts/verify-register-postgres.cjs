...
(async () => {
  ...
  try {
    ...
    assert.equal((await builder.branchContext(branchId,{id:actor,roleCode:'ADMIN'})).stateCode,'AP');
    await ds.query(`CREATE TABLE employees (employee_code text,name text,gender text,father_name text,date_of_birth date,date_of_joining date,designation text,phone text,uan text,pan text,esic text,aadhaar text,bank_account text,bank_name text,ifsc text,date_of_exit date,exit_reason text,client_id uuid,branch_id uuid,approval_status text,is_active boolean DEFAULT true)`);
    await ds.query(`INSERT INTO employees(employee_code,name,client_id,branch_id,approval_status) VALUES ('BRANCH-001','Selected branch employee',$1,$2,'APPROVED'),('OTHER-BRANCH','Other branch employee',$1,$3,'APPROVED'),('OTHER-CLIENT','Other client employee',$3,$2,'APPROVED'),('PENDING','Unapproved employee',$1,$2,'PENDING')`,[clientId,branchId,'44444444-4444-4444-8444-444444444444']);
    const employeeForm=REGISTER_FORMS.find(f=>f.sourceId==='apw'&&f.formNumber==='I').id;
    const scopedEmployees=await builder.prefill(employeeForm,branchId,'',2026,9,{id:actor,roleCode:'ADMIN'});
    assert.deepEqual(scopedEmployees.rows.map(r=>r.employee_1),['BRANCH-001']);
