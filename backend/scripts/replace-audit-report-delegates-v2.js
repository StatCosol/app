...
// updateStatus uses getLatestReportRow
lines = lines.map((l) =>
  l.includes('this.getLatestReportRow(')
    ? l.replace('this.getLatestReportRow(', 'this.reportService.getLatestReportRow(')
    : l,
);

// remove unused cache field
const cacheStart = findLine((l) => l.includes('private auditReportColumnsCache'));
if (cacheStart >= 0) {
  lines.splice(cacheStart, 6);
}

fs.writeFileSync(src, lines.join('\n'));
console.log('audits.service report delegates v2');
