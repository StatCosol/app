...
/**
 * PayDek Queries API adapter for the shared Thread UI.
 * Wraps /api/v1/notifications endpoints filtered by PAYDEK role context.
 */
@Injectable({ providedIn: 'root' })
export class PaydekThreadApiService implements ThreadApi {
  ...
  list(filters: ThreadFilters): Observable<PageRes<ThreadListItem>> {
    ...
    if (this.clientId) p = p.set('clientId', this.clientId);
  ...
  read(id: string): Observable<ThreadDetail> {
    return this.http.get<any>(`${this.base}/threads/${id}`).pipe(map((res) => this.toDetail(res)));
  }
  ...
  private toListItem(r: any): ThreadListItem {
    return {
      ...
      fromRole: r?.createdBy?.roleCode ?? 'PAYDEK',
      fromName: r?.createdBy?.name ?? '',
  ...
  private toDetail(res: any): ThreadDetail {
    ...
    return {
      ...
      messages: (msgs || []).map(
        (m: any): ThreadMessage => ({
          id: String(m?.id ?? ''),
          senderRole: m?.from?.roleCode ?? 'PAYDEK',
