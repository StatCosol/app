...
export class CcoThreadApiService implements ThreadApi {
  ...
  private toListItem(row: any): ThreadListItem {
    return {
      id: String(row?.id ?? ''),
      type: this.normalizeType(row?.queryType ?? row?.query_type),
      status: row?.status ?? 'OPEN',
      fromRole: this.normalizeRole(
        row?.createdBy?.roleCode ?? row?.from?.roleCode ?? 'CCO',
      ),
      ...
      messages: (messages || []).map(
        (message: any): ThreadMessage => ({
          ...
          senderRole: this.normalizeRole(
            message?.from?.roleCode ?? message?.sender?.roleCode ?? 'CCO',
          ),
