...
/**
 * LegitX (Client) Queries API adapter for the shared Thread UI.
 * Wraps /api/v1/notifications endpoints with creatorView (my threads).
 */
@Injectable({ providedIn: 'root' })
export class ClientThreadApiService implements ThreadApi {
  ...
  private toListItem(r: any): ThreadListItem {
    return {
      ...
      fromRole: r?.createdBy?.roleCode ?? 'CLIENT',
  ...
  private toDetail(res: any): ThreadDetail {
    ...
    return {
      ...
      messages: (msgs || []).map((m: any): ThreadMessage => ({
        id: String(m?.id ?? ''),
        senderRole: m?.from?.roleCode ?? 'CLIENT',
