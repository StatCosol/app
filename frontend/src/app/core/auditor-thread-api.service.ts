...
export class AuditorThreadApiService implements ThreadApi {
  ...
  private toListItem(row: any): ThreadListItem {
    ...
    const fromRole = this.normalizeRole(
      row?.createdBy?.roleCode ?? row?.from?.roleCode ?? 'AUDITOR',
    );
      ...
      messages: (messages || []).map(
        (message: any): ThreadMessage => ({
          ...
          senderRole: this.normalizeRole(
            message?.from?.roleCode ?? message?.sender?.roleCode ?? 'AUDITOR',
          ),
