...
export class ClientQueriesService {
  ...
  raiseQuery(body: any) {
    ...
    return this.http.post(`${this.baseUrl}/api/v1/notifications`, { ...body, subject });
  }

  listMyThreads() {
    return this.http.get(`${this.baseUrl}/api/v1/notifications/my`);
  }

  getThread(id: string) {
    return this.http.get(`${this.baseUrl}/api/v1/notifications/threads/${id}`);
  }

  reply(id: string, body: any) {
    return this.http.post(`${this.baseUrl}/api/v1/notifications/threads/${id}/reply`, body);
  }

  resolveThread(id: string) {
    // resolve in backend = close
    return this.http.post(`${this.baseUrl}/api/v1/notifications/threads/${id}/close`, {});
  }
}
