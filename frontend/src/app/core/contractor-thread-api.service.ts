...
/**
 * ConTrack (Contractor) Queries API adapter for the shared Thread UI.
 * Wraps /api/v1/notifications endpoints with creatorView (my threads).
 */
@Injectable({ providedIn: 'root' })
export class ContractorThreadApiService implements ThreadApi {
  ...
  list(filters: ThreadFilters): Observable<PageRes<ThreadListItem>> {
    ...
    return this.http.get<any>(`${this.base}/my`, { params: p }).pipe(
      map(res => ({
  ...
  private toListItem(r: any): ThreadListItem {
    return {
      ...
      fromRole: r?.createdBy?.roleCode ?? 'CONTRACTOR',
  ...
  private toDetail(res: any): ThreadDetail {
    ...
    return {
      ...
      messages: (msgs || []).map((m: any): ThreadMessage => ({
        id: String(m?.id ?? ''),
        senderRole: m?.from?.roleCode ?? 'CONTRACTOR',
