...
export class BranchSidebarComponent implements OnInit, OnDestroy {
  ...
  private moduleForRoute(route: string): string[] | null {
    ...
    if (route.startsWith('/branch/face-enrollment') || route.startsWith('/branch/facedesk-enrollment') || route.startsWith('/branch/face-failures')) return ['MOBILE_ATTENDANCE', 'CONTRACTOR_FACE_ATTENDANCE'];
