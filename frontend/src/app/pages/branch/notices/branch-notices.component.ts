...
@Component({
  standalone: true,
  selector: 'app-branch-notices',
  imports: [CommonModule, FormsModule, PageHeaderComponent, LoadingSpinnerComponent, EmptyStateComponent],
  template: `
    <div class="max-w-[1400px] mx-auto p-4">
      <ui-page-header title="Notices & Inspections" subtitle="Notices and inspection orders for your branch"></ui-page-header>

      <section class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4" *ngIf="kpis">
      ...
      <ui-empty-state *ngIf="!loading && !notices.length" title="No notices" description="No notices found for your branch."></ui-empty-state>
        ...
        <div *ngFor="let n of notices; trackBy: trackById"
          ...
          <div *ngIf="selected?.id === n.id && detail" class="mt-3 pt-3 border-t">
            <div class="grid grid-cols-2 gap-2 text-xs">
              ...
              <div><span class="text-gray-400">Response Due</span><br/>{{ detail.responseDueDate || '—' }}</div>
            </div>
...
export class BranchNoticesComponent implements OnInit, OnDestroy {
  ...
  constructor(private api: NoticesService, private cdr: ChangeDetectorRef) {}
  ngOnInit() { this.load(); this.api.branchKpis().pipe(takeUntil(this.destroy$)).subscribe({ next: k => { this.kpis = k; this.cdr.detectChanges(); } }); }
  ...
  load() {
    ...
    this.api.branchList(f).pipe(takeUntil(this.destroy$), timeout(15000), finalize(() => { this.loading = false; this.cdr.detectChanges(); }))
      .subscribe({ next: d => { this.notices = d; this.cdr.detectChanges(); } });
  }

  loadDetail(id: string) {
    this.api.branchGetOne(id).pipe(takeUntil(this.destroy$)).subscribe({ next: d => { this.detail = d; this.cdr.detectChanges(); } });
  }
