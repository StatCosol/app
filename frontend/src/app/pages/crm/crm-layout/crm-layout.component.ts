...
@Component({
  ...
  imports: [WorkspaceToolsComponent, RouterOutlet, CrmSidebarComponent, NewsTickerComponent, StatcoWordmarkComponent],
  template: `
    <div class="crm-shell">
      <!-- Mobile menu toggle -->
      <button
        class="lg:hidden fixed bottom-6 right-6 z-50 p-3.5 text-white rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 ring-4 ring-white/80 role-fab-crm"
...
<img
                ...
                <button
                  (click)="logout()"
                  class="inline-flex items-center gap-2 px-3.5 py-1.5 text-sm font-medium text-white rounded-lg hover:opacity-90 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 role-btn-crm"
                >
  ...
  styles: [`
    ...
    @media (min-width: 640px) { .crm-content { padding: 1.5rem 1rem; } }
    @media (min-width: 1024px) { .crm-content { padding: 2rem 1.5rem; } }
  `],
})
export class CrmLayoutComponent {
  ...
  constructor(private auth: AuthService, private ctxBoot: ComplianceContextBootstrapService) {
    this.ctxBoot.hydrate();
