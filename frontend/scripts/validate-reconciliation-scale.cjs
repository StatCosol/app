...
const sampleCount=Number(process.env.UI_REVIEW_ROWS || 5000);
assert.ok([1000,5000].includes(sampleCount));
run.employeeCount=sampleCount;
const baseline=Array.from({length:sampleCount},(_,i)=>({employeeCode:'EMP'+String(i).padStart(6,'0'),values:{gross_earnings:'25000.00',net_pay:'23200.00',pf_employee:'1800.00',esi_employee:'0.00'}}));
const csv='employee_code,period,gross_earnings,net_pay,pf_employee,esi_employee\n'+baseline.map(row=>row.employeeCode+',2026-09,25000.01,23200,1800,0').join('\n');
...
async function main(){
 ...
 try{
  ...
  const renderedRows=await page.$$eval('app-payroll-reconciliation details:not(.provenance)',rows=>rows.length);
  assert.equal(renderedRows,50,'Large reports must render only one page');
  await page.$$eval('app-payroll-reconciliation button',buttons=>buttons.find(button=>button.textContent.trim()==='Next').click());
  await page.waitForFunction(()=>document.querySelector('app-payroll-reconciliation').textContent.includes('Page 2'));
  const searchStarted=performance.now();
  await page.type('app-payroll-reconciliation input[type=search]','EMP'+String(sampleCount-1).padStart(6,'0'));
  await page.waitForFunction(()=>document.querySelectorAll('app-payroll-reconciliation details:not(.provenance)').length===1);
  const searchMs=performance.now()-searchStarted;
  assert.ok(await page.$eval('app-payroll-reconciliation details:not(.provenance) tbody',element=>element.textContent.includes('0.01')));
  fs.writeFileSync(path.join(output,'reconciliation-ui-'+sampleCount+'.json'),JSON.stringify({employees:sampleCount,renderedRows,reportMs:Number(reportMs.toFixed(2)),searchMs:Number(searchMs.toFixed(2)),scope:'Single local Chromium workflow sample with synthetic intercepted API responses, not production latency or percentile measurements.'},null,2));
  await page.$eval('app-payroll-reconciliation details:not(.provenance)',element=>{element.open=true;});
  await page.$eval('app-payroll-reconciliation',element=>element.scrollIntoView({block:'start'}));
  await page.screenshot({path:path.join(output,'reconciliation-ui-'+sampleCount+'-desktop.png')});
  ...
  await page.$eval('app-payroll-reconciliation',element=>element.scrollIntoView({block:'start'}));
  await page.screenshot({path:path.join(output,'reconciliation-ui-'+sampleCount+'-mobile.png')});
  ...
  console.log('PASS:',sampleCount,'employees; upload, 50-row pagination, next-page navigation, last-employee search, one-paisa difference and desktop/mobile layout.');
