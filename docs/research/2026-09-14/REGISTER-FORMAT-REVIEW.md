...
The current library contains 103 source-linked legal identities, of which 72 have implemented preparation layouts. It provides a research entry for each of the 28 states and eight union territories, with Central jurisdiction separate. This is **not complete national coverage**. Twenty-six original source PDFs are bundled; unsupported schedules remain reference-only. Generic unverified generation is disabled, while historical downloads remain available.

The user's priority is now **Telangana, Andhra Pradesh, Karnataka, Tamil Nadu, Kerala, Maharashtra, Haryana, Gujarat, Odisha, Madhya Pradesh, Delhi and Central Government rules**, covering Labour Acts as well as Labour Codes. See [the priority scope and acceptance checklist](PRIORITY-REGISTER-SCOPE.md). Twenty-nine of the implemented layouts fall within this group. Other jurisdictions remain available but are secondary for further expansion.

Review cut-off: 14 September 2026. The latest state-Shops/Act-selection increment passed 99 backend tests, 15 Angular tests and 23 isolated PostgreSQL checks. Both production builds passed. Earlier commit e83034d1 passed all 14 GitHub checks; checks for a subsequent commit must be verified separately. The sections below preserve the research and validation history; earlier batch counts are historical.
...
## State Shops records and client-as-contractor clarification

The user confirmed that portal account role is not the legal work relationship: a client company may itself act as a contractor to another company, and may have a separate principal-employer role when engaging its own vendors. The register selector now requires Act selection followed by Show registers. It displays exact Act identities only; switching Acts clears the previously prepared form. Same names/numbers and shared payroll data do not combine obligations. An integrated register does not automatically hide a contract-labour register.

Telangana G.O.Ms.No.6 (2 March 2019), notification paragraph 4(8), expressly cites the Shops and Establishments Act, 1988 and Rules 29/33. A separate TS_SHOPS_1988 binding enables both Forms II and III as one prepared workbook, with Form II entered once and all Form III worker columns, signatory and conditional principal-employer certification retained. The general MULTI_ACT reference is still not automatically activated for every predecessor Act. Publication/effectivity, exemptions, annual Form I and Code transition remain separate legal questions. Official later Shops instruments include registration exemptions in G.O.Ms.No.17 (13 June 2019) and the department index lists G.O.Rt.No.383 (23 September 2025); staff must review relevant establishment exemptions when confirming applicability.

The site's capacity must be explicitly selected: direct employer, principal employer or contractor. A client acting as contractor can use company-worker records without creating a self-vendor account; the other company's principal-employer particulars and representative are recorded. This context is an audited preparation input, not an inferred login privilege. Vendor selection, where used, retains the existing assigned-contractor access check. Both the particulars and capacity participate in duplicate detection and revision identity.

Primary source: https://ipass.telangana.gov.in/viewpdf.aspx?filepathnew=D%3A%2FTS-iPASSFinal%2Fdocs%2F2019LETF_MS6+%282%29.PDF (pages 1–2, 12–13). Later-instrument index: https://ipass.telangana.gov.in/Acts_Rules.aspx. Form III column 24 prints an inconsistent formula; the engine preserves this provenance note and never uses it as a wage-calculation formula. Additional recoveries need explanatory remarks and supporting evidence.

Maharashtra Shops Rules 2018 Form Q (Rule 26(1), page 90) now supports reviewed monthly attendance/wages, all 33 column groups and authentication; Form O (Rule 19, page 88) supports earned-leave accumulation, sanctioned/refused leave, discharge payments, festival and casual leave. Refusal dates/reasons and leave balances are checked. Q validates each month's dates, attendance totals, deduction components and net earnings including separately recorded overtime. Marathi/English and authentication requirements remain explicit. The three-year retention language in Rule 26(4) concerns Facilitator inspection records; it is not presented as a blanket three-year deletion rule for all registers. Rule 26(1)'s specific Minimum Wages Rule 27(1) alternative is not treated as automatic Code equivalence.

Primary source: https://mahakamgar.maharashtra.gov.in/Site/Upload/PDF/Regulation_of_Employment_Conditions_of_Service_Rules_2018.pdf (Rules 19/26 and pages 88/90). The November 2025 instrument found at https://labour.maharashtra.gov.in/en/notifications/intimation-closing-business is labelled draft by the government index; it is not used as a final amendment merely because the PDF heading resembles final rules. Current establishment applicability must be confirmed separately.

Migration 20260923_state_shops_registers.sql seeds separate state-Act applicability choices with no automatic enablement and is included in the pre-deployment service migration runner. Isolated PostgreSQL verifies repeat application, Code-only denial, header-order deduplication, revised particulars and client-as-contractor context. Fictional XLSX/PDF samples were inspected; Form II and III cannot be split into separate optional outputs, and printed worker identity repeats on continuation pages. This increment implements three layouts, not complete coverage of the priority states or complete contract-labour generation.

## Automatic branch state and employee scope

Register preparation now resolves the selected active branch through authorized client/branch access and uses its saved state code. The selector no longer defaults to Andhra Pradesh or offers unrelated states. Central rules are an additional choice only when that branch has confirmed Central appropriate-government facts. Missing or inconsistent state data requires profile correction; it does not select a fallback jurisdiction.
